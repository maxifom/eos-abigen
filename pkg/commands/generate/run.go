...
func generateTables(abi abitypes.ABI, contractName string, realContractName string, generatedFolder string, version string) error {
	...
	f.ImportAlias("github.com/maxifom/eos-abigen/pkg/client", "rpcClient")
	f.Const().Defs(
		Id("contractName").Op("=").Lit(realContractName),
	)
	f.Add(Type().Id("Client").Struct(
		Op("*").Qual("github.com/maxifom/eos-abigen/pkg/client", "RPCClient"),
	))

	constructor := Func().Id("NewClient").Params(Id("rpcClient").Op("*").Qual("github.com/maxifom/eos-abigen/pkg/client", "RPCClient")).Params(Id("*" + ("Client"))).Block(
	...
	for _, table := range abi.Tables {
		...
		if _, ok := declaredRowsNames[rowsName]; !ok {
			f.Type().Id(rowsName).Struct(
				Qual("github.com/maxifom/eos-abigen/pkg/base", "BaseRows"),
		...
		body.Id("options").Op("=").Append(
			Index().Qual("github.com/maxifom/eos-abigen/pkg/client", "RequestOption").Values(
				Qual("github.com/maxifom/eos-abigen/pkg/client", "Table").Call(Lit(table.Name)),
				Qual("github.com/maxifom/eos-abigen/pkg/client", "Scope").Call(Id("contractName")),
				Qual("github.com/maxifom/eos-abigen/pkg/client", "Code").Call(Id("contractName")),
				Qual("github.com/maxifom/eos-abigen/pkg/client", "Limit").Call(Lit(-1)),
			),
		...
		id := Func().
			...
			Id(tableActionName).
			Params(Id("ctx").Qual("context", "Context"), Id("options").Op("...").Qual("github.com/maxifom/eos-abigen/pkg/client", "RequestOption")).
	...
	for _, abiStruct := range abi.Structs {
		...
		for _, field := range abiStruct.Fields {
			...
			case "bool":
				...
				fieldGen.Op("*").Qual("github.com/maxifom/eos-abigen/pkg/base", "Bool")
			case "int8":
				if isList {
					fieldGen.Index()
				}
			...
			case "uint64":
				...
				fieldGen.Op("*").Qual("github.com/maxifom/eos-abigen/pkg/base", "UInt64")
			case "int128", "uint128":
				if isList {
					fieldGen.Index()
				}
				fieldGen.Op("*").Qual("github.com/maxifom/eos-abigen/pkg/base", "BigInt")
			case "float32":
				if isList {
					fieldGen.Index()
				}
				fieldGen.Op("*").Qual("github.com/maxifom/eos-abigen/pkg/base", "Float32")
			case "float64":
				if isList {
					fieldGen.Index()
				}
			...
			case "extended_asset":
				...
				fieldGen.Qual("github.com/maxifom/eos-abigen/pkg/base", "ExtendedAsset")
			default:
